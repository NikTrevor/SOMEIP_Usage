...
void notify(){
    ...
    const vsomeip::byte_t its_data[] = { 0x50, 0x40, 0x30, 0x20, 0x10 };
    payload = vsomeip::runtime::get()->create_payload();
    payload->set_data(its_data, sizeof(its_data));

    std::set<vsomeip::eventgroup_t> its_groups;
    its_groups.insert(SAMPLE_EVENTGROUP_EXTRA_ID);
    app->offer_event(SAMPLE_SERVICE_EXTRA_ID, SAMPLE_INSTANCE_ID, SAMPLE_EVENT_EXTRA_ID, its_groups);
    app->notify(SAMPLE_SERVICE_EXTRA_ID, SAMPLE_INSTANCE_ID, SAMPLE_EVENT_EXTRA_ID, payload);
}
...
int main() {
    ...
    app->offer_service(SAMPLE_SERVICE_EXTRA_ID, SAMPLE_INSTANCE_ID);
